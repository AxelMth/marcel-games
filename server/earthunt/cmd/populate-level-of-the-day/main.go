...
// run harmless: the next one simply finds fewer gaps. At thirty days the
// schedule can lapse for a month before a player ever notices — which is
// exactly what happened when it lapsed between May and August 2026 and the
// daily challenge started arriving empty.
...
func ensureLevel(ctx context.Context, date time.Time) (bool, error) {
	...
	if err == nil && existing != nil && len(existing.CountryCodes) > 0 {
		return false, nil
	}

	// Same date-seeded generator the API falls back to, so a day written here
	// and a day rebuilt there are the same puzzle.
	countryCodes := utils.DailyLevelCountryCodes(day)
	if _, err := repositories.CreateLevelOfTheDay(ctx, day, countryCodes); err != nil {
		return false, err
	}
	return true, nil
}
