...
// levelRankQuery classe une partie parmi celles du même niveau et du même mode.
...
func countBetterOnLevel(
	...
	var rows []rankCount
	err := db.Client().Prisma.
		QueryRaw(levelRankQuery, gameMode, level, timeSpent, attempts).
		Exec(ctx, &rows)
	if err != nil {
		return 0, err
	}
