...
/** WordClimb's own registration. Serving its ads here would be a policy breach. */
const OTHER_APP_IDS = [
  "ca-app-pub-6271901101573718~8478860820",
  "ca-app-pub-6271901101573718~9736102177",
]
...
describe("AdMob identifiers are EarthHunt's own", () => {
  ...
  it("does not reuse WordClimb's app registration", () => {
    // Serving one app's ads from another can get the whole AdMob account
    // limited, and it corrupts the other app's metrics.
    for (const id of Object.values(ADMOB_APP_IDS)) {
      expect(OTHER_APP_IDS).not.toContain(id)
    }
  })
